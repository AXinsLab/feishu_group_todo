...
2. 新任务提取：
   - 显性任务：消息中明确指派（如"让/请 XXX 做 YYY"、"XXX 负责 YYY"）
   - 隐性任务：从对话上下文中推断，以下情况均应提取为任务：
     * 某人被请求或被询问某件事（即使措辞婉转，如"这个需要 XXX 评估一下"）
     * 某人表示需要评估、确认、跟进、排查某问题
     * 某人做出承诺（如"我去看看"、"好的，我来处理"）
   - 任务描述应完整概括背景和待做事项，不超过 50 字
   - 语义去重：若新任务与现有任务语义相同或高度相似，\
不得重复新增，只更新状态

3. 负责人识别优先级：
   a. 消息中有飞书 @ 提及（取 mention_open_id）
   b. 对话中被明确要求/请求的人（被对话指向的一方）
   c. 群成员表模糊匹配（匹配英文名、名字的"名"部分、群昵称，不区分大小写）
   d. LLM 上下文推断（谁做出承诺、谁被委托）
   e. 兜底：归属消息发送者本人（sender），进展备注标注"负责人待确认"
